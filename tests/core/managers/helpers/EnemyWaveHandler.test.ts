...
vi.mock('../../../../src/core/utils/Logger', () => ({
    ...
    default: { // Also mock the singleton instance if used directly elsewhere
        log: mockLoggerLog,
        debug: mockLoggerDebug,
        warn: mockLoggerWarn,
        error: vi.fn(),
...
vi.mock('../../../../src/core/events/EventBus', () => ({
    ...
    default: { // Also mock the singleton instance
         emit: mockEventBusEmit,
         on: vi.fn(),
         off: vi.fn(),
         cleanup: vi.fn(),
...
describe('EnemyWaveHandler', () => {
    ...
    beforeEach(() => {
        ...
        mockEnemyManager = new (vi.mocked(EnemyManager) as any)(); // Instantiate mocked EnemyManager
    ...
    describe('Wave Spawning', () => {
        ...
        it('should spawn boss on boss wave frequency', () => {
             ...
             handler = new EnemyWaveHandler(mockEnemyManager, mockDifficultyConfig, mockEventBus, mockLogger); // Re-init
             ...
             // Check the calls made during the last advanceWave (which triggered wave 3 spawn)
             expect(mockEnemyManagerSpawnEnemy).toHaveBeenCalledWith(mockDifficultyConfig.bossId, expect.any(Object));
        ...
        it('should spawn enemies in a grid pattern for regular waves', () => {
            ...
            expect(mockEnemyManagerSpawnEnemy).toHaveBeenCalledWith('triangle', { x: expect.any(Number), y: 100 }); // First row
            expect(mockEnemyManagerSpawnEnemy).toHaveBeenCalledWith('triangle', { x: expect.any(Number), y: 150 }); // Second row
            expect(mockEnemyManagerSpawnEnemy).toHaveBeenCalledWith('triangle', { x: expect.any(Number), y: 200 }); // Third row
        });
    });
